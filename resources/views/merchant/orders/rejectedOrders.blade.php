        ...
        <div class="col-md-12">
            <div class="main-card mb-3 card">
                <div class="card-body">
                    ...
                    <form action="{{ url('merchant/filterRejectedOrders') }}" method="GET">
                        {{ csrf_field() }}
                        <div class="form-row">
                            @if ($errors->any())
                                ...
                                <table dir="rtl" style="width: 100%;" id="example"
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($orders as $o)
                                            ...
                                            <tr id="removable{{ $o->id }}">
                                                <td>{{ $count }}</td>
                                                <td><a
                                                        href="{{ url('merchant/order/' . $o->id) }}">#{{ $o->id }}</a>
                                                </td>
                                                <td>
                                                    @if (!empty($o->client))
                                                        {{ $o->client->name }}@endif
                                            ...
                                            @endforeach --}}
                                                ...
                                                <td>{{ $o->created_at }}</td>
                                                <td>
                                                    @if ($o->delivery_confirmed_by_runner == 0 && $o->state == 'rejected' && $o->merchant_reject_reason != '')
                                                        {{ $o->merchant->name }}
                                                        @endif
                                                        <br>
                                                        @if ($o->delivery_confirmed_by_client == 0 && $o->state == 'rejected' && $o->client_reject_reason != '')
                                                         العميل
                                                        @endif
                                                ...
                                                <td>
                                                    @if ($o->delivery_confirmed_by_runner == 0 && $o->state == 'rejected' && $o->merchant_reject_reason != '')
                                                    {{ $o->merchant_reject_reason }}
                                                    @endif
                                                    <br>
                                                    @if ($o->delivery_confirmed_by_client == 0 && $o->state == 'rejected' && $o->client_reject_reason != '')
                                                    {{ $o->client_reject_reason }}
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    <!-- --accept item or refuse it  -->{{-- @if ($o->state == 'accepted')
                                           <a href="javascript:void(0);" onclick="deliveredOrder({{$o->id}});" title="تسليم الطلب" class="btn btn-secondary btn-sm ">
                                                تسليم
                                            </a>
