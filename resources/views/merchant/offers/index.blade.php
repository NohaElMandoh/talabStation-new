...
<div class="row">
    <div class="col-md-12">
        <div class="main-card mb-3 card">
            <div class="card-body">
                <div>
                    <a href="{{url('merchant/offer/create')}}" class="btn btn-primary">
                            ...
                            <table dir="rtl" style="width: 100%;" id="example" class="table table-hover table-striped table-bordered dataTable dtr-inline" role="grid" aria-describedby="example_info">
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" style="width: 187.2px;" aria-label="Offer_Merch: activate to sort column ascending">السعر</th>
                                    ...
                                    <tr id="removable{{$offer->id}}">
                                        ...
                                        <td>@if(!empty($offer->photo_url))
                                            <a href="{{url($offer->photo)}}" target="_blank">
                                                <img class="img-index" src="{{url($offer->photo_url)}}" height="50" title="offer image">
                                            </a>
                                            @endif
                                        </td>
                                        <td>{{date('d-m-Y', strtotime($offer->starting_at))}}
                                          </td>
                                        <td>{{date('d-m-Y', strtotime($offer->ending_at))}}
                                          </td>
                                        <td>@if($offer->available ==true) متاح @else غير متاح @endif</td>

                                        <td>
                                            <!-- --show offer details -->
                                            <a href="{{url('merchant/offer/'.$offer->id)}}" class="btn btn-info btn-sm" title="عرض التفاصيل"> <i class="fa fa-eye"></i></a>
                                            <!-- --edit offer  -->
                                            <a href="{{ url('merchant/offer/'.$offer->id.'/edit') }}" class="btn btn-success btn-sm ">
