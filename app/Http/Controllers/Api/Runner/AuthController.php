...
class AuthController extends Controller
{
    public function register(Request $request)
    {
        $validation = validator()->make($request->all(), [
            'name' => 'required',
            'phone' => 'required|unique:runners,phone',
            'region_id' => 'required',
            'address' => 'required',
            'email' => 'required|unique:runners,email',
        ...
        $user = Runner::create($request->all());
        if ($request->hasFile('photo')) {
            $path = public_path();
            $destinationPath = $path . '/uploads/runners/'; // upload path
            $logo = $request->file('photo');
            $extension = $logo->getClientOriginalExtension(); // getting image extension
            $name = time() . '' . rand(11111, 99999) . '.' . $extension; // renameing image
            $logo->move($destinationPath, $name); // uploading file to given path
            $user->update(['photo' => 'uploads/runners/' . $name]);
        }
        if ($user) {
            $data = [
                'api_token' => $userToken,
    ...
    public function profile(Request $request)
    {
        $validation = validator()->make($request->all(), [
            'password' => 'confirmed',
            Rule::unique('runners')->ignore($request->user()->id),
        ]);
        if ($validation->fails()) {
    ...
    public function login(Request $request)
    {
        ...
        $user = Runner::where('email', $request->input('email'))->first();
        if ($user) {
    ...
    public function reset(Request $request)
    {
        ...
        $user = Runner::where('email', $request->email)->first();
        if ($user) {
            ...
            if ($update) {
                ...
                Mail::send('emails.reset', ['code' => $code], function ($mail) use ($user) {
                    $mail->from('no-reply@ipda3.com', 'تطبيق البياع');
    ...
    public function password(Request $request)
    {
        ...
        $user = Runner::where('code', $request->code)->where('code', '!=', 0)->first();
